...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'ground':
				{
					defaultCamZoom = 1.0;
					curStage = 'ground';
					var sky:FlxSprite = new FlxSprite(-350, -230).loadGraphic(Paths.image('ground/Sun', 'week8'));
					sky.scrollFactor.set(0.1, 0.1);
					add(sky);

					var groundd:FlxSprite = new FlxSprite(-400, -300).loadGraphic(Paths.image('ground/Ground', 'week8'));
					groundd.setGraphicSize(Std.int(groundd.width * 1.15));
					add(groundd);
				}
			case 'mirror':
				{
					defaultCamZoom = 0.8;
					var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('bg', 'week8'));
					bg.setGraphicSize(Std.int(bg.width * 1.10));
					add(bg);
				}
		}
		...
		healthBar.createFilledBar(0xFFFF0000, 0xFF1D9ED1);
	...
	function week8Intro(?dialogueBox:DialogueBox):Void 
	{
		switch (SONG.song.toLowerCase()) 
			{
				case 'under':
				...
				case 'deceiver':
					...
					dad.animation.play('intro');
					boyfriend.animation.play('intro');
	...
	override public function update(elapsed:Float) //UPDATE OVERRIDE
	{
		#if html5
		...
		if (barUp != null && barUp.velocity.y != 0 && barUp.y >= -220)
			{
				barUp.velocity.y = 0;
			}
		
			if (barDw != null && barDw.velocity.y != 0 && barDw.y <= 600)
			{
				barDw.velocity.y = 0;
			}	
		...
		if (curSong == 'under') 
			{
				...
				switch (curStep)
				{
					case 480:
						trace("inicio");
						barUp = new FlxSprite(-100, -600).loadGraphic(Paths.image('Bar', 'week8'));
						barUp.velocity.y = 550;
						

						barUp.scrollFactor.set(0.2, 0.2);
						barUp.antialiasing = true;

						barDw = new FlxSprite(-100, 1000).loadGraphic(Paths.image('Bar2', 'week8'));
						barDw.velocity.y = -550;

						
						barDw.scrollFactor.set(0.1, 0.1);
						barDw.antialiasing = true;
